...
ggplot(data = df, aes(x = trend, y = vals, group = apps)) +
  ...
  labs(x = "Months (March 2020=0)", y = "Number of active users",
       caption = "Note: dotted horizontal lines denote begning of a given COVID-19 wave in Poland. The first is March 2020, the second is August 2020\nand the third is January 2021. Red dashed line denote pre-COVID-19 trend (before March 2020)") -> p1
p1
```
...
space_state_results %>% 
  ...
  bind_rows(.id = "apps") %>%
  rownames_to_column() %>%
  filter(str_detect(rowname, "Aver")) %>%
  dplyr::select(apps, 
                abs=AbsEffect, abs_l=AbsEffect.lower, abs_u=AbsEffect.upper, 
                rel=RelEffect, rel_l=RelEffect.lower,  rel_u=RelEffect.upper) %>%
  mutate_at(vars(rel:rel_u), function(x) x*100) %>%
  mutate_at(vars(abs:abs_u), function(x) as.integer(round(x))) -> average_over_period
...
space_state_results %>%
  ...
  map_df(~.x %>% as.data.frame() %>%
           ...
           filter(month == "2020-09-15"), .id = "apps") %>%
...
ggplot(data = covid_poland, aes(x = date, y = new_cases, group = 1)) +
  ...
  geom_vline(xintercept = as.Date("2020-03-12"), linetype = "dotted", color = "black") + 
  geom_vline(xintercept = as.Date("2020-09-30"), linetype = "dotted", color = "black") +
